      ...
      if (res?.error) {
        if (res.error === 'EmailNotVerified' || res.error.includes('EmailNotVerified')) {
          toast.error('Kérjük, erősítsd meg az email címedet a belépéshez!', {
            action: {
              label: 'Újraküldés',
              onClick: () => router.push(`/profile?resend=${encodeURIComponent(email)}`)
            }
          })
    ...
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#121212] border border-white/5 rounded-2xl p-8 shadow-2xl">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div className="relative">
            ...
            <input
              ...
              autoComplete="email"
              className="w-full bg-[#0a0a0a] border border-white/10 rounded-xl p-4 pl-12 text-white focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none transition-all"
            />
          </div>
          <div>
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-[#0a0a0a] border border-white/10 rounded-xl p-4 pl-12 pr-12 text-white focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none transition-all"
              />
              <button
                type="button"
        ...
        <button
          ...
          disabled={isLoading || isGoogleLoading}
          className="w-full py-4 bg-white text-black font-bold rounded-xl transition-all flex items-center justify-center gap-2 hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]"
        >
          {isGoogleLoading ? (
