...
import { Plus, Trash2, Tag, Edit, Save, X, Loader2 } from 'lucide-react'
...
export default function CategoriesClient({ initialCategories }: { initialCategories: Category[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let res
      if (editingId) {
        res = await updateCategoryAction(editingId, categoryData)
      } else {
        res = await createCategoryAction(categoryData)
      }
